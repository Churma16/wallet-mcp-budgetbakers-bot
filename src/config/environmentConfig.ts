...
export function loadEnvironmentConfiguration(): ApplicationEnvironmentConfiguration {
  ...
  const geminiFallbackModels = (process.env.GEMINI_FALLBACK_MODELS || 'gemini-3.5-flash,gemini-3.5-flash-lite')
    ...
    .filter(Boolean);
  const geminiRequestTimeoutSeconds = Number.parseInt(process.env.GEMINI_TIMEOUT_SECONDS || '20', 10) || 20;
  ...
  const aiFallbackModels = (process.env.AI_FALLBACK_MODELS || '')
    ...
    .map(modelItem => modelItem.trim())
    .filter(Boolean);
  const aiRequestTimeoutSeconds = Number.parseInt(
  ...
  const logRetentionDays = Number.parseInt(process.env.LOG_RETENTION_DAYS || '7', 10) || 7;
  const emailSyncEnabled = process.env.EMAIL_SYNC_ENABLED === 'true';
  const emailImapHost = process.env.EMAIL_IMAP_HOST || 'imap.gmail.com';
  const emailImapPort = Number.parseInt(process.env.EMAIL_IMAP_PORT || '993', 10) || 993;
  const emailImapUser = process.env.EMAIL_IMAP_USER || '';
  const emailImapPassword = process.env.EMAIL_IMAP_PASSWORD || '';
  const emailLookbackMinutes = Number.parseInt(process.env.EMAIL_LOOKBACK_MINUTES || '10', 10) || 10;
  ...
  const whatsappMaxReconnectAttempts = Number.parseInt(process.env.WHATSAPP_MAX_RECONNECT_ATTEMPTS || '6', 10) || 6;
  const whatsappReconnectMaxBackoffSeconds = Number.parseInt(process.env.WHATSAPP_RECONNECT_MAX_BACKOFF_SECONDS || '300', 10) || 300;
  const whatsappMessageQueueIntervalMs = Number.parseInt(process.env.WHATSAPP_MESSAGE_QUEUE_INTERVAL_MS || '1000', 10) || 1000;
  const parsedTypingCooldown = process.env.WHATSAPP_TYPING_PRESENCE_COOLDOWN_MS !== undefined
    ? Number.parseInt(process.env.WHATSAPP_TYPING_PRESENCE_COOLDOWN_MS, 10)
    : 2500;
  const whatsappTypingPresenceCooldownMs = Number.isNaN(parsedTypingCooldown) ? 2500 : parsedTypingCooldown;

  const telegramMaxStartupAttempts = Number.parseInt(process.env.TELEGRAM_MAX_STARTUP_ATTEMPTS || '5', 10) || 5;
  const telegramStartupRetryDelayMs = Number.parseInt(process.env.TELEGRAM_STARTUP_RETRY_DELAY_MS || '2000', 10) || 2000;

  const parsedMaxMediaDownloadMb = Number.parseInt(process.env.MAX_MEDIA_DOWNLOAD_MB || '10', 10);
