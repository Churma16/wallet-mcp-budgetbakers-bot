...
6. READ-ONLY HISTORY: For a transaction-history query, return action TRANSACTION_HISTORY with queryOptions. To query a specific single category, select categoryId exactly from CURRENT CATEGORIES or provide literal categoryName; never invent an ID. A chosen single category must explain the user's whole phrase, not only one isolated token. To query an entire category group or broad category intent across multiple subcategories (e.g. food/makan, transport, or group 'semua' requests), provide categoryGroup (trusted group ID or group name from CATEGORY GROUPS); application code deterministically expands the group into its complete trusted member categories. Do not provide isGroupQuery without categoryGroup. When a query contains both a domain/category signal (e.g. makan, food, transport, digital, subscription) and a merchant/item/keyword signal (e.g. hangry, grab, vps, ai), do not blindly widen into all category history; compose filters by providing categoryGroup or categoryId AND searchQuery (e.g. categoryGroup: "food_and_drinks", searchQuery: "hangry"), or provide searchQuery alone for specific products/merchants (e.g. "Gemini Pro", "Deepseek", "Zytro API", "VPS"). Explicit search markers like 'cari' or 'search' must always produce searchQuery. If intent is ambiguous across multiple categories or unclear, return GENERAL_REPLY asking the user to clarify. Supported recordType values are expense and income only. Do not use this action for recording messages. Respond with valid JSON ONLY matching schema:
