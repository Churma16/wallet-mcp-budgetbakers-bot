...
export type PreparedSemanticHistoryPrompt = {
  promptText: string; systemInstruction: string; requestContextDescription: string;
};
...
export function validateSemanticHistoryQueryResponse(value: unknown, tokenUsage?: TokenUsageStatistics): SemanticHistoryQueryResult {
  ...
  for (const field of ['limit', 'page'] as const)
    if (options[field] !== undefined && (!Number.isInteger(options[field]) || Number(options[field]) <= 0))
      throw new Error(`Invalid semantic history field '${field}'.`);
...
export function prepareSemanticHistoryPrompt(
  userMessageText: string, availableAccountList: WalletAccountItem[],
  availableCategoryList: WalletCategoryItem[], referenceInstant: Date
  ...
  return {
    systemInstruction: buildSemanticHistorySystemInstruction(
      ...
      getCurrentLocalDateString(referenceInstant, timezone), timezone
    ),
...
export async function executeSemanticHistoryWorkflow(options: {
  ...
  const prepared = prepareSemanticHistoryPrompt(
    options.userMessageText, options.availableAccountList,
    options.availableCategoryList, options.referenceInstant || new Date()
  );
